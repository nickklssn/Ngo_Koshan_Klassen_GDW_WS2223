...
async function getCarData(carId) {
  ...
  async function calcData(driverId, carId) {
    ...
    return {"budget": budget, "liter": fuelAmount, "fueltype": carData.fuelType, "carName": carData.name, "maxRange": range.maxRange, "minRange": range.minRange}
  }
  ...
  async function writeSightsJSON(driverId) {
    ...
    let maxRange = await (await calcData(driverId, driverData.carId)).maxRange;
    let minRange = await (await calcData(driverId, driverData.carId)).minRange;
